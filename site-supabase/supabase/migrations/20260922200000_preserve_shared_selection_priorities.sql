-- Preserva a prioridade principal/alternativa nos links de leitura.
-- Contrato aditivo: links existentes sem d continuam sendo opções principais.
...
begin
  ...
  loop
    ...
    if jsonb_typeof(v_item) <> 'object'
      ...
      or (v_item ? 'd' and (jsonb_typeof(v_item -> 'd') <> 'string' or v_item ->> 'd' <> 'alternative'))
      or (v_has_kit and (
...
comment on column site_private.shared_selections.items is
  'Referências compartilhadas por link; preserva prioridade, grupo, nome e multiplicadores de kit. Nomes escolhidos pelo cliente também ficam visíveis ao destinatário.';
