-- Integridade do contexto no Site Promo Brindes.
...
alter table site_private.quote_items
  add column if not exists variant_id_snapshot text
    check (variant_id_snapshot is null or variant_id_snapshot ~ '^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,99}$');
...
begin
  ...
  select id, request_hash into v_request_id, v_existing_hash from site_private.contact_requests where client_request_id = v_client_request_id;
  if v_request_id is not null then
    if v_existing_hash <> v_request_hash then raise exception using errcode = 'P0001', message = 'client_request_id_conflict'; end if;
    return jsonb_build_object('requestId', v_request_id, 'duplicate', true);
  end if;
  ...
  insert into site_private.contact_requests (
    client_request_id, request_hash, source, contact_name, email, phone, message, preferred_channel,
    page_url, client_submitted_at, request_metadata
  ) values (
    v_client_request_id, v_request_hash, p_payload ->> 'source', p_payload #>> '{contact,name}', lower(p_payload #>> '{contact,email}'),
    nullif(p_payload #>> '{contact,phone}', ''), nullif(p_payload #>> '{contact,message}', ''), nullif(p_payload #>> '{contact,responseChannel}', ''),
    nullif(p_payload ->> 'pageUrl', ''), (p_payload ->> 'submittedAt')::timestamptz, p_request_meta - 'identifierHash' - 'requestHash'
  ) on conflict (client_request_id) do nothing returning id into v_request_id;
  ...
  insert into site_private.consent_receipts (request_kind, contact_request_id, accepted, notice_version, client_accepted_at)
  values ('contact', v_request_id, true, p_payload #>> '{consent,noticeVersion}', (p_payload #>> '{consent,acceptedAt}')::timestamptz);
  return jsonb_build_object('requestId', v_request_id, 'duplicate', false);
end;
$$;
...
comment on column site_private.quote_items.variant_id_snapshot is
  'Identificador publicado da variante selecionada, preservado no retrato do briefing.';
comment on column site_private.contact_requests.preferred_channel is
  'Preferência opcional de retorno; não representa consentimento para disparo automático.';
