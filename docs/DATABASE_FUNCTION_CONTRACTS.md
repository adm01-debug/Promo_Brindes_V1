...
| `invalid_occasion_id` | `set_my_occasion_favorite` | `src/lib/customerOccasionFavorites.ts` normaliza e rejeita IDs fora do formato editorial antes da RPC; resposta remota malformada vira indisponibilidade do recurso | erro na seleção de datas; sem gravar |
| `occasion_favorite_limit_reached` (`errcode P0001`) | `set_my_occasion_favorite` | Limite transacional de 100 datas por conta; a interface mantém o estado anterior e informa que a sincronização não foi concluída | erro na seleção de datas |
