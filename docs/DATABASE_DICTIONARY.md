...
Gerado por `npm run db:site:dictionary` a partir de `pg_description` no banco local em 2026-09-16. Não editar à mão — a fonte de verdade é o comentário na migration (`comment on table`/`comment on column`); rode o script de novo depois de qualquer mudança de schema.
