...
As migrations `20260908230000_create_site_lead_storage.sql`, `20260909103000_lock_down_rls_event_trigger.sql` e `20260909180000_create_customer_quote_portal.sql` foram aplicadas no projeto exclusivo após aprovação do proprietário. As duas primeiras tiveram somente os identificadores históricos normalizados de 8 para 14 dígitos em 15/09/2026, sem reaplicação do SQL. Reconciliação confirmada em 16/09/2026 por `SUPABASE_WORKDIR=site-supabase npx supabase@latest migration list`: as 18 versões batem entre local e remoto (nenhuma com o formato antigo), e `db push --dry-run` reporta o remoto atualizado. As variáveis server-side, endpoints relativos e configurações públicas da Área do Cliente estão configurados na Vercel; os formulários estão ativos em <https://promo-brindes-v1.vercel.app>.
