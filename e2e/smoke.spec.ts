...
test('falha ao abrir proposta fica visível e permite nova tentativa', async ({ page }) => {
  ...
  await page.route('**/auth/v1/user', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify(user) }));
  await page.route('**/rest/v1/rpc/claim_my_quote_requests', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ claimed: 0 }) }));
  await page.route('**/rest/v1/rpc/get_my_quote_request', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ id: quoteId, protocol: '77777777', status: 'quoted', company: 'Empresa Proposta', contactName: 'Bia', email: user.email, phone: '(11) 98888-8888', city: null, desiredDeadline: null, notes: null, createdAt: '2026-09-09T12:00:00Z', submittedAt: '2026-09-09T12:00:00Z', items: [{ key: `${product.id}::preto`, productId: product.id, slug: product.slug, name: product.name, sku: product.sku, imageUrl: product.primary_image_url, quantity: 50, minQuantity: 50, colorName: null, colorHex: null }], events: [], proposals: [{ id: proposalId, version: 1, title: 'Proposta para campanha', validUntil: '2020-01-01', publishedAt: '2026-09-09T14:00:00Z', isCurrent: true }] }) }));
...
test('detalhe do orçamento recupera de indisponibilidade temporária', async ({ page }) => {
  ...
  await page.route('**/rest/v1/rpc/get_my_quote_request', (route) => {
    ...
    return route.fulfill({ contentType: 'application/json', body: JSON.stringify({ id: quoteId, protocol: '99999999', status: 'new', company: 'Empresa Recuperada', contactName: 'Caio', email: user.email, phone: '(11) 97777-7777', city: null, desiredDeadline: null, notes: null, createdAt: '2026-09-09T12:00:00Z', submittedAt: '2026-09-09T12:00:00Z', items: [{ key: `${product.id}::sem-cor`, productId: product.id, slug: product.slug, name: product.name, sku: product.sku, imageUrl: product.primary_image_url, quantity: 50, minQuantity: 50, colorName: null, colorHex: null }], events: [], proposals: [] }) });
  });
...
test('falha ao trocar de orçamento nunca mantém dados e ações da rota anterior', async ({ page }) => {
  ...
  await page.route('**/rest/v1/rpc/get_my_quote_request', (route) => {
    ...
    return route.fulfill({ contentType: 'application/json', body: JSON.stringify({ id: firstId, protocol: 'ROTA-1', status: 'new', company: 'Empresa da rota anterior', contactName: 'Dani', email: user.email, phone: '(11) 96666-6666', city: null, desiredDeadline: null, notes: null, createdAt: '2026-09-09T12:00:00Z', submittedAt: '2026-09-09T12:00:00Z', items: [{ key: `${product.id}::sem-cor`, productId: product.id, slug: product.slug, name: product.name, sku: product.sku, imageUrl: product.primary_image_url, quantity: 50, minQuantity: 50 }], events: [], proposals: [] }) });
  });
