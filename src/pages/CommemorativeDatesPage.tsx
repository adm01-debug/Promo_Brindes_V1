    ...
    <>
      ...
      <section className="dates-planner section" id="agenda" aria-labelledby="dates-planner-title">
        <div className="container">
          ...
          {view === 'calendar' && month !== null ? <CalendarGrid year={year} month={month} occasions={results} onOpen={openDetail} /> : results.length ? <div className="occasion-list">{results.map((occasion) => <OccasionCard key={occasion.id} occasion={occasion} favorite={favorites.has(occasion.id)} onFavorite={() => toggleFavorite(occasion)} onOpen={() => openDetail(occasion)} />)}</div> : <div className="dates-empty"><CalendarDays size={42} /><span>AGENDA ABERTA</span><h2>Nenhuma data combina com esses filtros.</h2><p>Experimente outro público, navegue por todo o ano ou conte sua ocasião para nosso time de especialistas.</p><div><button className="button button--dark" type="button" onClick={() => { setSearch(''); setParams({ ano: String(year) }, { replace: true }); }}>Ver o ano inteiro</button><Link className="text-link" to="/contato">Planejar uma data própria <ArrowRight size={17} /></Link></div></div>}
        </div>
      </section>
