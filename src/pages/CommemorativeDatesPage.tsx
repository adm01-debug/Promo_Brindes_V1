import { ArrowRight, BellRing, Bookmark, BookmarkCheck, CalendarDays, Clock3, Download, Gift, HeartHandshake, Lightbulb, Search, Share2, Sparkles, Users, X } from 'lucide-react';
...
import { useOccasionFavorites } from '../lib/useOccasionFavorites';
import {
  filterOccasions,
  monthNames,
  nextOccasion,
...
export default function CommemorativeDatesPage() {
  ...
  const { favorites, saveFavorite, storageMessage } = useOccasionFavorites({
    userId: auth.user?.id,
    authLoading: auth.loading,
    knownOccasionIdsKey: occasionIds,
  });
  ...
  useEffect(() => setLastRemovedFavorite(null), [auth.user?.id]);
  ...
  function setFavorite(occasion: DatedOccasion, nextSaved: boolean, allowUndo = true) {
    if (!saveFavorite(occasion.id, nextSaved, () => setLastRemovedFavorite(null))) return;
      ...
      <section className="dates-planner section" id="agenda" aria-labelledby="dates-planner-title">
        <div className="container">
          ...
          <div className="dates-toolbar">
            ...
            <div className="dates-view" role="group" aria-label="Visualização da agenda"><button type="button" className={view === 'list' ? 'is-active' : ''} aria-pressed={view === 'list'} onClick={() => updateParams({ visualizacao: null })}><span aria-hidden="true">≡</span> Lista</button><button type="button" className={view === 'calendar' ? 'is-active' : ''} aria-pressed={view === 'calendar'} onClick={() => updateParams({ visualizacao: 'calendario', mes: month === null ? now.getMonth() + 1 : month + 1 })}><span aria-hidden="true">▦</span> Calendário</button></div>
          </div>
