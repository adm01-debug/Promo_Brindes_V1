  ...
  return (
    <>
      ...
      <section className="section shared-selection"><div className="container">
        ...
        {!loading && !error && items.length > 0 && <><div className="shared-selection__heading"><div><span>{items.length} {items.length === 1 ? 'referência atual' : 'referências atuais'}</span><h2>{totalUnits(items).toLocaleString('pt-BR')} unidades estimadas</h2></div><p>{unavailableProducts.length || unavailableVariants.length ? 'Revise as referências indisponíveis antes de duplicar esta seleção.' : 'Você pode duplicar esta base e ajustar quantidades antes de pedir uma proposta.'}</p></div><div className="shared-selection__grid">{items.map((item) => <article key={item.key}><img src={item.imageUrl} alt="" width="180" height="180" referrerPolicy="no-referrer" /><div><p>Cód. {item.sku}</p><h2>{item.name}</h2>{item.kitGroupId && <span>{item.kitName} · {item.kitQuantity} kits × {item.unitsPerKit} un.</span>}{item.colorName && <span>{item.colorName}</span>}{item.variantUnavailable && <span>Variante não publicada</span>}</div><strong>{item.quantity.toLocaleString('pt-BR')} un.</strong><Link to={`/produto/${item.slug}`}>Ver produto <ArrowRight size={16} /></Link></article>)}</div></>}
      </div></section>
