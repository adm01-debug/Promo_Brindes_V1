...
export default function SharedSelectionPage() {
  ...
  const requiresSelectionReview = Boolean(unavailableProducts.length || unavailableVariants.length || invalidKitReferences.length);
  ...
  function applyDuplicate() {
    if (loading || error || !items.length || requiresSelectionReview) return;
  ...
  return (
    <>
      ...
      <section className="section shared-selection"><div className="container">
        ...
        {!loading && !error && invalidKitReferences.length > 0 && <aside className="shared-selection__privacy shared-selection__privacy--warning" role="alert"><PackageOpen size={19} /><p><strong>A composição do kit precisa de revisão.</strong> Produtos ou mínimos mudaram desde a criação do link. Peça uma seleção atualizada para preservar a composição e as quantidades.</p></aside>}
        ...
        {!loading && !error && !items.length && <div className="shared-selection-state"><PackageOpen size={40} /><h2>Os produtos deste link não estão mais publicados.</h2><p>O catálogo muda; explore as alternativas disponíveis agora.</p><Link className="button button--dark" to="/catalogo">Ver alternativas</Link></div>}
        {!loading && !error && items.length > 0 && <><div className="shared-selection__heading"><div><span>{items.length} {items.length === 1 ? 'referência atual' : 'referências atuais'}</span><h2>{totalUnits(items).toLocaleString('pt-BR')} unidades estimadas</h2></div><p>{requiresSelectionReview ? 'Revise as referências sinalizadas e peça um link atualizado antes de duplicar esta seleção.' : 'Você pode duplicar esta base e ajustar quantidades antes de pedir uma proposta.'}</p></div><div className="shared-selection__grid">{items.map((item) => <article key={item.key}><img src={item.imageUrl} alt="" width="180" height="180" referrerPolicy="no-referrer" /><div><p>Cód. {item.sku}</p><h2>{item.name}</h2>{item.decisionGroup === 'alternative' && <span>Alternativa para comparar</span>}{item.kitGroupId && <span>{item.kitName} · {item.kitQuantity} kits × {item.unitsPerKit} un.</span>}{item.colorName && <span>{item.colorName}</span>}{item.variantUnavailable && <span>Variante não publicada</span>}</div><strong>{item.quantity.toLocaleString('pt-BR')} un.</strong><Link to={`/produto/${item.slug}`}>Ver produto <ArrowRight size={16} /></Link></article>)}</div></>}
      </div></section>
