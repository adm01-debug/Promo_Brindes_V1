...
function QuoteContent() {
  ...
  async function downloadProposal(proposal: CustomerProposal) {
    ...
    } catch { setError('Não conseguimos abrir esta proposta agora.'); }
    finally { setDownloading(''); }
  }

  if (loading) return <div className="customer-state container" role="status">Carregando solicitação…</div>;
  if (error && !quote) return <div className="customer-state container"><span>SOLICITAÇÃO</span><h1>Não foi possível abrir.</h1><p>{error}</p><Link className="button button--dark" to="/minha-conta">Voltar ao histórico</Link></div>;
