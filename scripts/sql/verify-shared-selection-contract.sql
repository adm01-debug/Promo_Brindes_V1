...
begin
  ...
  v_read := public.get_site_shared_selection(v_token);
  if v_read->'items' is distinct from v_items then
    raise exception 'shared_selection_roundtrip_failed';
  end if;
  perform public.revoke_site_shared_selection(v_token, repeat('a',64));
  if public.get_site_shared_selection(v_token) is not null then
